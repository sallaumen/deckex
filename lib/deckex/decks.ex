defmodule Deckex.Decks do
  ...
  **The `Deck:` header is not decoration.** `Deckex.Decks.DecklistParser` walks
  ...
  # Everything derived from the list goes stale when the list moves. The
  # dossier says so and waits to be regenerated by hand; the combos refresh
  # themselves, because that costs one request to a public API rather than a
  # consult, and the job debounces a burst of edits into one.
  defp list_changed(%Deck{} = deck) do
